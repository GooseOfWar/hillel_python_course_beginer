...
Затем используя функцию1 (вызвать ее) написать функцию2 которая будет принимать 3 аргумента и находить максимум
с помощью функции1.
в итоге будет .
...
user_data: list = input('Fillup three numbers. Use space as a separator.\n').split()  # user data input

# check for data compatibility
# user_data shall contain 3 arguments and all argument are numbers
# for start checking all criteria are False
...
while not all([quantitative, all_numbers]):  # Work while all criteria not True
    ...
    if len(user_data) < 3:  # If list length less than 3 - error message and input again
        user_data: list = input('Data shall three numbers. Try again and use space as a separator.\n').split()
    else:
        quantitative: bool = True  # quantitative criteria is True if OK

    if not all([i.isdigit() for i in user_data]):  # If data isn`t digit - error message and input again
        user_data: list = input('Data shall be only digit. Try again and use space as a separator.\n').split()
    else:
        all_numbers: bool = True  # all_numbers criteria is True if OK
else:
    operating_data: list = [int(i) for i in user_data]  # create operating_data list with converted data in int


def max_form_2(arg_1: int, arg_2: int) -> int:  # return the max arg from 2 args
